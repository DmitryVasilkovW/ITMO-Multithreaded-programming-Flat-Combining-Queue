...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun <R> flatCombine(taskElement: Any?, action: () -> R): R {
        ...
        while (true) {
            if (combinerLock.compareAndSet(false, true)) {
                return taskCombiner(isInTasks, taskIdx, action)
            }
            if (isInTasks) {
    ...
    private fun cas(taskElement: Any?, taskIdx: Int): Boolean {
        return tasksForCombiner.compareAndSet(
            taskIdx,
            null,
            taskElement
        )
    ...
    private fun <R> taskCombiner(isInTasks: Boolean, taskIdx: Int, action: () -> R): R {
